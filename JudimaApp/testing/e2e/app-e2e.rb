...
Selenium::WebDriver::Chrome.driver_path="/mnt/c/Users/Restop-2345/unneeded/ruby/chromedriver.exe"
    ...
    scenario %{create acct} do
      ...
      my_proc = Proc.new do |*args|
        ...
        unless response_body[%{QR_code}]== nil
          post_backend_response << response_body
        end
        response
      end
      ...
      pass_input.fill_in :with => %{Turkey123456}
      confirm_pass_input.fill_in :with => %{Turkey123456}
      ...
      post_backend_requests = Billy.proxy.requests.select do |x|
        hash_body = Hash.new
        begin
          hash_body = JSON.parse x[:body]
        rescue
          nil
        end
        x[:method] == 'POST' && x[:url] == 'http://localhost:3005/' && hash_body[%{env}] == %{QR_init}
      end
      .at 0
      # pp post_backend_requests
      pp post_backend_response

      # p TablePrint::Printer.table_print(Billy.proxy.requests, [
      #   :status,
      #   :handler,
      #   :method,
      #   { url: { width: 100 } },
