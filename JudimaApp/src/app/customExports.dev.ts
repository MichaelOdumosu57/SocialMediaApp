...
export type zBools = "divBoard" |
    ...
    "gsap-cursor"|"icon"|"app-components"
    // dev additions
...
export interface zProtoChildren {
    ...
    options?:{
        ...
        extras?:zOptionsExtras |
        {
            appVanillaTilt?:{
                type:String,
                group:String,
                options?:any // Vanilla Tilt init options
            } | {
                confirm:"true",
                type:"body",
                zSymbolNeeded:"true",
    ...
    navigation?:{
        ...
        name:Array<string>,
        confirm?:string, //enum "true"
        type:Array<string> //enum "body"
    }  | {
        group:String
    ...
    delta?:{
        group: Array<{
            name: String,
            type: "add_remove_button" |  "repeat" | "cdn",
            by?: String | Number
        }>
    }  | {
        group:string ,
    ...
    latch?:{
        ...
        zChildren?:Array<{
            ...
            //     "deltaNodeContainer"
            // }>
            needed?:Array<string> //["appLatchMetadata";
            extras?:any
            group:Array<String>
        }>
    }
...
export function stack(
    ...
    if(type === 'simpleStack'){
		...
		if(zChild["&#8353"].extras?.judima?.moving?.type === "custom" && mediaQuery !== "mobile"){
            ...
            if(target === undefined){
                throw("provided a signature that corresponds to an componentObject before it")
            }
            else if([
                ryber[target].metadata.board.top,
                ryber[target].metadata.board.height]
                .includes(undefined)
            ){
                throw(`provide 2 dummy elements the full width of the board for your formatIgnore zChild
                whats happening is the algorithm needs zChildren for finalAlign but needs more in order not to
                ignore your trying to prevent errors with dummy zChildren`)
            }
			let diff
			switch (point) {
				case "right":
...
export let judimaDirective:{
    onInit:Function,
    onDestroy:Function,
    onDestroyConnectedMesg:number
} = {
    onInit:(devObj)=>{
        let {myThis,featureFn,loadedScripts} = devObj
        loadedScripts = loadedScripts || []
        let {extras,appExtras,ryber,zChildren, subscriptions} = myThis
        let { co } = extras
        let group, suffix
        try{
            group   = ryber[co].metadata[appExtras.name].group
            suffix  = ryber[co].metadata[appExtras.name].suffix
        }
        catch(e){
            throw("update app.component.ts with the new group acoording to the schema")
        }
    ...
    onDestroy:(devObj)=>{
        let {myThis} = devObj
        let {extras,appExtras,ryber} = myThis
        if(extras?.confirm === "true"){
            ...
            if(ryber.appCO0.metadata[appExtras.name] && judimaDirective.onDestroyConnectedMesg < 1){
                console.log(`you night have to apply the ngOnDestroy BehaviorSubject to the app.component directive object and use it
                to help your types know when to start to activate themselves, you might be activating
                types of a new components when type of an old are still unsubscribing.
                The simplest way to get this done is to push the subject onto the loaded scripts array
                This issue ariese when going to the previous component and not the next
                `)
                judimaDirective.onDestroyConnectedMesg += 1
            }
            if(ryber.appCO0.metadata[appExtras.name]?.ngOnDestroy){
                ryber.appCO0.metadata[appExtras.name].ngOnDestroy.next({
                    hook:"finished"
                })
            }
